            ...
            for run in range(0, runs):
                ...
                if os.path.isfile(evalFile):
                    if(hfo): #HFO experiment
                        ...
                        #try:
                        _et, _es, _eub = np.loadtxt(open(evalFile, "r"), skiprows=1, delimiter=",", unpack=True)
                        #except:
                        #    continue
                        if sum(evalTrials)==0:
                            evalTrials = _et
...
def summarize_experiment_data(source,hfo=True,compact=False):
    ...
    for value in values:
        ...
        #print(evalFile)
        evalFileContent = np.loadtxt(open(evalFile, "r"), skiprows=1, delimiter=",", unpack=True)
...
def draw_graph(source1 = None, name1 = "Algo1", significant1=None,
    ...
    if source1 != None:
        ...
        summary1Content = np.loadtxt(open(summary1File, "r"), skiprows=1, delimiter=",", unpack=True)
    ...
    if source2 != None:
        ...
        summary2Content = np.loadtxt(open(summary2File, "r"), skiprows=1, delimiter=",", unpack=True)
    ...
    if source3 != None:
        ...
        summary3Content = np.loadtxt(open(summary3File, "r"), skiprows=1, delimiter=",", unpack=True)
    ...
    if source4 != None:
        ...
        summary4Content = np.loadtxt(open(summary4File, "r"), skiprows=1, delimiter=",", unpack=True)
    ...
    if source5 != None:
        ...
        summary5Content = np.loadtxt(open(summary5File, "r"), skiprows=1, delimiter=",", unpack=True)
    ...
    if source6 != None:
        ...
        summary6Content = np.loadtxt(open(summary6File, "r"), skiprows=1, delimiter=",", unpack=True)
...
def get_args():
    ...
    parser.add_argument('-s','--source',default='/var/local/fleno/gitProjects/Distributional_HFO/log/C51Agent')
    parser.add_argument('-r','--runs',type=int, default=50)
    return parser.parse_args()

def main():
    parameter = get_args()
    collect_experiment_data(parameter.source, parameter.runs,compact=False)
    summarize_experiment_data(parameter.source,compact=False)
